...
def month_to_indian_season(month: int) -> str:
    if month in (12, 1, 2):
        return "Winter"
    if month in (3, 4, 5, 6):
        return "Summer"
...
def load_data() -> pd.DataFrame:
    ...
    df["day_of_week_number"] = df["datetime"].dt.dayofweek
    df["day_of_year"] = df["datetime"].dt.dayofyear
    df["quarter"] = df["datetime"].dt.quarter
    df["date"] = df["datetime"].dt.date
    df["is_weekend"] = df["datetime"].dt.dayofweek >= 5
    df["season"] = df["month"].apply(month_to_indian_season)
    return df
...
def season_bar(df: pd.DataFrame, title: str) -> go.Figure:
    grouped = df.groupby("season", as_index=False)["demand"].mean()
    grouped["season"] = pd.Categorical(grouped["season"], SEASON_ORDER, ordered=True)
    ...
    fig.update_layout(height=390, margin=dict(l=20, r=20, t=55, b=20), showlegend=False)
    return fig


def season_month_heatmap(df: pd.DataFrame, title: str) -> go.Figure:
    pivot = df.pivot_table(index="season", columns="month", values="demand", aggfunc="mean")
...
def eda_notebook_tab(df: pd.DataFrame, outputs: dict) -> None:
    timestamp = outputs["timestamp"]
    inferential = outputs["inferential"]
    ...
    st.markdown("### Notebook-Style EDA Walkthrough")
    st.caption(
        "This tab mirrors the executed EDA notebook: load data, inspect quality, validate timestamps, "
        "study patterns, check relationships, and review model/lag results."
    )

    metric_row(
        [
            ("Rows", f"{len(df):,}", None),
            ("Columns", "6 source cols", None),
            ("Interval", "15 min", "regular"),
            ("Null Cells", str(int(df[["datetime", "demand", "block"]].isna().sum().sum())), None),
        ]
    )

    with st.expander("1. Data Preview and Schema", expanded=True):
        ...
        with right:
            schema = pd.DataFrame(
                {
                    "column": ["datetime", "date", "time", "block", "demand", "entrydatetime"],
                    "role": [
                        "main timestamp",
                        "date field",
                        "time field",
    ...
    with st.expander("2. Missing Values, Timestamp Checks, and Demand Validity", expanded=True):
        demand_checks = timestamp.get("demand_checks", {})
        metric_row(
            [
                ...
                ("Zero Demand", str(demand_checks.get("zero_demand_rows", 0)), None),
                ("Negative Demand", str(demand_checks.get("negative_demand_rows", 0)), None),
            ]
        )
    ...
    with st.expander("3. Descriptive Statistics and Demand Distribution", expanded=True):
        desc = df[["block", "demand", "hour", "month", "year", "day_of_year"]].describe().T
        ...
        with left:
            ...
            st.plotly_chart(fig, width="stretch", key="eda_demand_histogram")
        with right:
            fig = px.box(
                df,
                x="demand",
                title="Demand Boxplot",
                color_discrete_sequence=["#f2a93b"],
            )
            fig.update_layout(height=380)
            st.plotly_chart(fig, width="stretch", key="eda_demand_boxplot")

    with st.expander("4. Time Trend and Bivariate Patterns", expanded=True):
        st.plotly_chart(demand_line_chart(df), width="stretch", key="eda_daily_demand_trend")
        col1, col2 = st.columns(2)
        with col1:
            st.plotly_chart(grouped_bar(df, "hour", "Average Demand by Hour", "#3867d6"), width="stretch", key="eda_hour_bar")
            st.plotly_chart(grouped_bar(df, "block", "Average Demand by Block", "#f2a93b"), width="stretch", key="eda_block_bar")
        with col2:
            st.plotly_chart(grouped_bar(df, "month", "Average Demand by Month", "#12a886"), width="stretch", key="eda_month_bar")
            st.plotly_chart(season_bar(df, "Average Demand by Indian Season"), width="stretch", key="eda_season_bar")
            corr = load_csv("bivariate_multivariate_output/bivariate_correlations.csv")
            if not corr.empty:
                st.dataframe(corr, width="stretch", hide_index=True)

    with st.expander("5. Multivariate Correlation", expanded=True):
        feature_columns = [
            "block",
            "hour",
            "minute",
        ...
        st.plotly_chart(fig, width="stretch", key="eda_multivariate_corr_heatmap")

    with st.expander("6. Inferential and ML Results", expanded=True):
        ci = inferential.get("confidence_interval_mean_demand_95pct", {})
        weekday = inferential.get("weekday_vs_weekend_tests", {})
        metric_row(
            [
                ("Mean CI Lower", f"{ci.get('lower', 0):.2f}", None),
                ("Mean CI Upper", f"{ci.get('upper', 0):.2f}", None),
                ("Weekday Mean", f"{weekday.get('weekday_mean', 0):.2f}", None),
                ("Weekend Mean", f"{weekday.get('weekend_mean', 0):.2f}", None),
            ]
        )
        metric_row(
            [
                ...
                ("Regression R2", f"{regression.get('r2', 0):.4f}", None),
                ("Regression MAE", f"{regression.get('mae', 0):.2f}", None),
            ]
        )

    with st.expander("7. Lag Feature Analysis", expanded=True):
        ...
        with col1:
            ...
            if not lag_corr.empty:
                st.dataframe(lag_corr, width="stretch", hide_index=True)
        with col2:
            st.markdown(
                """
                <div class="insight">
                Recent demand is the strongest forecasting signal. The previous 15-minute demand value has
                the highest relationship with current demand, followed by the 30-minute lag and rolling 1-hour mean.
                </div>
                """,
                unsafe_allow_html=True,
            )
...
def overview_tab(df: pd.DataFrame, outputs: dict) -> None:
    ...
    st.plotly_chart(demand_line_chart(df), width="stretch", key="overview_daily_demand_trend")

    left, right = st.columns([1.1, 0.9])
    with left:
        st.markdown("### Demand Shape")
        st.plotly_chart(grouped_bar(df, "hour", "Average Demand by Hour", "#3867d6"), width="stretch", key="overview_hour_bar")
    with right:
...
def data_quality_tab(outputs: dict) -> None:
    ...
    with left:
        ...
        if not interval_df.empty:
            ...
            st.plotly_chart(fig, width="stretch", key="quality_interval_distribution")
    with right:
        null_df = load_csv("timestamp_data_quality_output/null_counts.csv")
        if not null_df.empty:
            fig = px.bar(
            ...
            st.plotly_chart(fig, width="stretch", key="quality_null_counts")
...
def patterns_tab(df: pd.DataFrame) -> None:
    col1, col2 = st.columns(2)
    with col1:
        st.plotly_chart(grouped_bar(df, "block", "Average Demand by Block", "#f2a93b"), width="stretch", key="patterns_block_bar")
        st.plotly_chart(grouped_bar(df, "month", "Average Demand by Month", "#12a886"), width="stretch", key="patterns_month_bar")
        st.plotly_chart(season_bar(df, "Average Demand by Indian Season"), width="stretch", key="patterns_season_bar")
    with col2:
        ...
        st.plotly_chart(fig, width="stretch", key="patterns_weekday_bar")
        ...
        if not corr.empty:
            ...
            st.plotly_chart(fig, width="stretch", key="patterns_bivariate_corr")

        st.plotly_chart(
            season_month_heatmap(df, "Season and Month Demand Heatmap"),
            width="stretch",
            key="patterns_season_month_heatmap",
        )
...
def models_tab(outputs: dict) -> None:
    ...
    with col1:
        ...
        if not confusion.empty:
            ...
            st.plotly_chart(fig, width="stretch", key="models_confusion_matrix")
    with col2:
        ...
        if not importance.empty:
            ...
            st.plotly_chart(fig, width="stretch", key="models_regression_importance")
...
def lag_tab(outputs: dict) -> None:
    ...
    with col1:
        ...
        if not lag_corr.empty:
            ...
            fig.update_yaxes(categoryorder="total ascending")
            st.plotly_chart(fig, width="stretch", key="lag_correlation_bar")
    with col2:
        ...
        if not lag_importance.empty:
            ...
            fig.update_yaxes(categoryorder="total ascending")
            st.plotly_chart(fig, width="stretch", key="lag_importance_bar")
    ...
    if not predictions.empty:
        ...
        st.plotly_chart(fig, width="stretch", key="lag_actual_vs_prediction")
...
def summary_tab() -> None:
    ...
    if summary_path.exists():
        ...
        summary_text = "Work Done Process summary is not available yet."

    with st.expander("Work Done Process", expanded=True):
...
def main() -> None:
    ...
    with tabs[0]:
        eda_notebook_tab(filtered, outputs)
    with tabs[1]:
